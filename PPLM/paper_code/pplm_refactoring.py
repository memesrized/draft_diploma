...
def perturb_past(past, model, prev, args, classifier, stepsize=0.01, vocab_size=50257,
                 original_probs=None, accumulated_hidden=None, true_past=None, grad_norms=None, device="cpu"):
...
def latent_perturb(model, args, context=None, sample=True, device='cpu', enc=None):
    # Make agnostic loading
    classifier = ClassificationHead(class_size=2, embed_size=1024).to(device)
    classifier.load_state_dict(torch.load("discrim_models/toxic_classifierhead_CUSTOM.pt", map_location=torch.device(device)))
    classifier.eval()
    ...
    original, _, _ = sample_from_hidden(model=model, args=args, context=context, device=device,
                                  perturb=False, classifier=classifier, enc=enc)
...
def sample_from_hidden(model, args, classifier, context=None, past=None, device='cpu',
    ...
    for i in trange(args.length, ascii=True):
        ...
        # first time from latent_pertrub
        ...
        if not perturb or args.num_iterations == 0:
            ...
            perturbed_past, _, grad_norms, loss_per_iter = perturb_past(past, model, prev, args, stepsize=current_stepsize,
        ...
        output_text = enc.decode(output.tolist()[0])
        print(output_text)
        if output_text.endswith("<|endoftext|>"):
            break
