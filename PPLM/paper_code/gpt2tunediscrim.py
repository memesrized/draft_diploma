...
tokenizer = GPT2Tokenizer.from_pretrained('/home/memesrized/projects/education/diploma/models/paraphrase/')

model = GPT2LMHeadModel.from_pretrained('/home/memesrized/projects/education/diploma/models/paraphrase/')
...
class Discriminator(torch.nn.Module):
    def __init__(self):
        ...
        self.spltoken = self.spltoken.to(device_)
...
class Discriminator2mean(torch.nn.Module):
    ...
    def forward(self, x):
        mask_src = 1 - x.eq(0).unsqueeze(1).type(torch.FloatTensor).to(device_).detach()
...
def collate_fn(data):
    def merge(sequences):
        ...
        padded_seqs = torch.zeros(len(sequences), max(lengths)).long().to(device_)  # padding index 0
    ...
    return x_batch, torch.tensor(y_batch, device=device_, dtype=torch.long)


def train_epoch(data_loader, discriminator, device=device_, args=None, epoch=1):
...
def test_epoch(data_loader, discriminator, device=device_, args=None):
    discriminator.eval()
    test_loss = 0
    correct = 0
    with torch.no_grad():
...
def main():
    ...
    if args.dataset_label == 'SST':
       ...
       # with open("data/train_discrim_unidecode.jsonl") as file:
        with open("data/DISCRIM_TRAIN_DATASET.jsonl") as file:
    ...
    for epoch in range(args.epochs):
        ...
        #             "discrim_models/{}_mean_lin_discriminator_{}_CUSTOM.pt".format(args.dataset_label, epoch))
        torch.save(discriminator.get_classifier().state_dict(),
                    "discrim_models/{}_classifierhead_CUSTOM_{}.pt".format(args.dataset_label, epoch))
