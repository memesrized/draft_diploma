...
def latent_perturb(model, args, context=None, sample=True, device='cpu', enc=None):
    if args.discrim == 'clickbait':
        ...
        classifier = ClassificationHead(class_size=2, embed_size=1024).to(device)
        classifier.load_state_dict(torch.load("discrim_models/toxicity_classifierhead.pt", map_location=torch.device(device)))
        classifier.eval()
    ...
    original, _, _ = sample_from_hidden(model=model, args=args, context=context, device=device,
                                  perturb=False, good_index=good_index, classifier=classifier, enc=enc)
...
def sample_from_hidden(model, args, classifier, context=None, past=None, device='cpu',
                       sample=True, perturb=True, good_index=None, enc=None):
